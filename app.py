...
# Global team metadata (updated after bootstrap)
TEAM_SHORT: Dict[int, str] = {}
TEAM_LOGO_URL: Dict[int, str] = {}  # team_id -> logo url
...
def derive_next_and_fdr(fixtures: List[dict], player_team_id: Optional[int], n_fix=6):
    """
    Return (labels list, fdr list) like ['MCI(H)', ...] up to n_fix,
...
def run_analysis():
    ...
    with tabs[1]:
        ...
        outs = outs.sort_values("out_score", ascending=False).head(6)
        ...
        top_in = cand.sort_values("in_score", ascending=False).head(6)
    ...
    with tabs[2]:
        ...
        # Include FDRs to color Next1..Next6
        adv_cols_visible = [
